...
class Console {
  public:
	...
	struct RegisterVariableProxy {
		RegisterVariableProxy(Console &console) : m_console(console) {}
	...
	struct RegisterCommandProxy {
		RegisterCommandProxy(Console &console) : m_console(console) {}
	...
	void registerSpecialCommand(const std::string &name, int index,
  ...
  public:
	// Exposed for testing
	std::list<std::string> const &getLog() const {
		return log;
	}
...
struct ConsoleAddLines {
	ConsoleAddLines(Console &console_) : console(console_) {}
