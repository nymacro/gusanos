...
#define READ_W(x_)                                                                                                     \
	{                                                                                                                  \
		for (size_t i = 0; i < sizeof(w) / sizeof(*w); ++i)                                                            \
			f.read((char *)&w[i].x_, sizeof(w[i].x_));                                                                 \
	}
#define READ_O(x_)                                                                                                     \
	{                                                                                                                  \
		for (size_t i = 0; i < sizeof(o) / sizeof(*o); ++i)                                                            \
			f.read((char *)&o[i].x_, sizeof(o[i].x_));                                                                 \
	}
...
static unsigned char const waveHeader[] = {
	82, 73, 70, 70, 2,	236, 0, 0, 87, 65, 86, 69, 102, 109, 116, 32, 16,  0,  0,	0,
	1,	0,	1,	0,	34, 86,	 0, 0, 34, 86, 0,  0,  1,	0,	 8,	  0,  100, 97, 116, 97,
};
...
string WeapSettings::writeWeapon() {
	...
	f << "  shoot_particles(" << writeWObj() << ", " << (int)parts << ", " << velocityFactor(speed) << ", 0, "
...
string WeapSettings::writeWObj() {
	...
	if (idx == 29) // Laser
		f << "repeat = 10000"
		  << endl; // TODO: Fix a bug in gusanos, objects keep getting processed after they get destroyed
