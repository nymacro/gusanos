...
class Cache {
  public:
	Cache(ConstructorT const &construct_ = ConstructorT(), DestructorT const &destruct_ = DestructorT())
		: construct(construct_), destruct(destruct_) {}
	...
	struct Item {
		Item(ValueT const &value_, int lifetime_) : lifetime(lifetime_), value(value_) {}
