...
inline bool iisPrefixOfOther(std::string::const_iterator a, std::string::const_iterator ae,
							 std::string::const_iterator b, std::string::const_iterator be) {
	for (; a != ae && b != be; ++a, ++b) {
...
inline bool istrCmp(IterA a, IterA ae, IterB b, IterB be) {
	for (; a != ae && b != be; ++a, ++b) {
		char ca = std::toupper(*a);
		char cb = std::toupper(*b);
		if (ca != cb)
...
inline bool istrCmp(char const *a, IterB b, IterB be) {
	for (; *a && b != be; ++a, ++b) {
		char ca = std::toupper(*a);
		char cb = std::toupper(*b);
		if (ca != cb)
...
std::string shellComplete(ContainerT const &items, std::string::const_iterator b, std::string::const_iterator e,
						  GetText getText, ShowAlternatives showAlternatives) {
	size_t len = e - b;
	...
	if (lastMatch == firstMatch) {
		...
		for (;; ++i) {
			for (item = firstMatch; item != lastMatch; ++item) {
				if (item->size() <= i || tolower((*item)[i]) != tolower((*firstMatch)[i])) {
