...
struct Location {
	...
	Location(Location const &l) : file(l.file), line(l.line) {}

	Location(std::string const &file_, int line_) : file(&file_), line(line_) {}
...
#define LUA_WLOG_ONCE(x_)                                                                                              \
	if (context.logOnce(std::cout))                                                                                    \
...
#define DLOG(x_)                                                                                                       \
	if (logOptions.debug) {                                                                                            \
		(std::cout << __FILE__ ":" << __LINE__ << ": " << x_ << std::endl);                                            \
	} else                                                                                                             \
		(void)0
#define DLOGL(l_, x_)                                                                                                  \
	if (logOptions.debug) {                                                                                            \
		...
		(void)0
#define TLOG(x_)                                                                                                       \
	if (logOptions.level >= LOG_TRACE) {                                                                               \
		(std::cout << __FILE__ ":" << __LINE__ << ": " << x_ << std::endl);                                            \
	} else                                                                                                             \
		(void)0
#define WLOG(x_)                                                                                                       \
	if (logOptions.level >= LOG_WARNINGS) {                                                                            \
		(std::cerr << __FILE__ ":" << __LINE__ << ": " << x_ << '\n');                                                 \
	} else                                                                                                             \
		(void)0
#define ILOG(x_)                                                                                                       \
	if (logOptions.level >= LOG_INFO) {                                                                                \
		(std::cerr << x_ << '\n');                                                                                     \
	} else                                                                                                             \
		(void)0
#define ELOG(x_)                                                                                                       \
	if (logOptions.level >= LOG_ERRORS) {                                                                              \
		(std::cerr << __FILE__ ":" << __LINE__ << ": " << x_ << '\n');                                                 \
	} else                                                                                                             \
		(void)0
#else
