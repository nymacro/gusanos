...
class BasicAngle {
  public:
	...
	explicit BasicAngle(T data_) : data(data_) {}

	explicit BasicAngle(double degrees) : data(T(degrees * (double(count) / 360.0))) {}
...
class BasicAngleReplicator : public ZCom_ReplicatorBasic {
  ...
  public:
	BasicAngleReplicator(ZCom_ReplicatorSetup *setup, Type *data) : ZCom_ReplicatorBasic(setup), m_ptr(data) {
