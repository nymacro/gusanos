...
struct BaseVec {
	...
	template <class T2>
	explicit BaseVec(BaseVec<T2> const &b) : x(static_cast<T>(b.x)), y(static_cast<T>(b.y)) {}

	BaseVec(BaseVec const &a, BaseVec const &b) : x(b.x - a.x), y(b.y - a.y) {}

	BaseVec(T const &x_, T const &y_) : x(x_), y(y_) {}
