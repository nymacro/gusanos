...
class BitArray {
  public:
	struct Segment {
		...
		Segment(size_t pos_, size_t len_, bool val_) : pos(pos_), len(len_), val(val_) {}
	...
	struct Scanner {
		Scanner(BitArray &array_, SegmentIter segm_) : array(array_), segm(segm_) {}
