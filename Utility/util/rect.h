...
class BasicRect {
  public:
	...
	BasicRect(T x1_, T y1_, T x2_, T y2_) : x1(x1_), y1(y1_), x2(x2_), y2(y2_) {}

	BasicRect(BITMAP *b) : x1(0), y1(0), x2(b->w - 1), y2(b->h - 1) {}
	...
	bool isIntersecting(BasicRect const &b) const {
		if (b.y2 < y1 || b.y1 > y2 || b.x2 < x1 || b.x1 > x2) {
			return false;
		} else {
			return true;
		}
	...
	bool isInside(T x, T y) const {
		...
		return diffX < getWidth() && diffX >= T(0) && diffY < getHeight() && diffY >= T(0);
	}
