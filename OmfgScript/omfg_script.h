...
struct TokenBase {
	...
	TokenBase(Location loc_) : loc(loc_) {}
...
struct Parser : public Pimpl<ParserImpl> {
	...
	std::list<TokenBase *> const &getList(std::string const &name,
										  std::list<TokenBase *> const &def = std::list<TokenBase *>());
