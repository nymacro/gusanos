...
struct ParamDef {
	struct Param {
		Param(std::string const &name_, bool optional_ = true) : name(name_), optional(optional_) {}
...
struct Parameters {
	...
	Parameters(ParamDef *def, Location loc_)
		: cur(0), flags(0), paramDef(def), params(def->params.size(), 0), loc(loc_) {}
	...
	void addParam(std::string const &name, unique_ptr<OmfgScript::TokenBase> v, Location loc_) {
		...
		if (i >= paramDef->params.size()) {
			...
			if (minParam >= 0 && minDist <= static_cast<int>((name.size() + 1) / 2))
				error("Unknown parameter '" + name + "'. Did you mean '" + paramDef->params[minParam].name + "'?",
					  loc_);
			else
		...
		for (size_t i = 0; i < params.size(); ++i) {
			if (!params[i]) {
				if (!paramDef->params[i].optional)
					error(S_("Parameter '")
						  << paramDef->params[i].name << "' (number " << (i + 1) << ") not defined and not optional");
				else
...
struct List : public TokenBase {
	List(Location loc_) : TokenBase(loc_) {}
...
Function::Function(Location loc_, std::string const &name_) : TokenBase(loc_), name(name_) {}
...
struct Func : public Function {
	Func(Location loc_, std::string const &name_) : Function(loc_, name_) {}
...
struct BinOp : public TokenBase {
	BinOp(Location loc_, TokenBase *a_, TokenBase *b_) : TokenBase(loc_), a(a_), b(b_) {}
	...
	virtual Type::type type() {
		...
		if (at == bt) {
			return at;
		} else if ((at == Type::Double && bt == Type::Int) || (at == Type::Int && bt == Type::Double))
			return Type::Double;
		return Type::Unknown;
	}
...
struct Add : public BinOp {
	Add(Location loc_, TokenBase *a_, TokenBase *b_) : BinOp(loc_, a_, b_) {}
...
struct Sub : public BinOp {
	Sub(Location loc_, TokenBase *a_, TokenBase *b_) : BinOp(loc_, a_, b_) {}
...
struct Mul : public BinOp {
	Mul(Location loc_, TokenBase *a_, TokenBase *b_) : BinOp(loc_, a_, b_) {}
...
struct Div : public BinOp {
	Div(Location loc_, TokenBase *a_, TokenBase *b_) : BinOp(loc_, a_, b_) {}
...
ParamProxy::ParamProxy(ParamDef *paramDef_) : paramDef(paramDef_) {}
...
ActionFactory::ActionFactory() : Pimpl<ActionFactoryImpl>(new ActionFactoryImpl()) {}
...
struct ParserImpl : public TGrammar<ParserImpl> {
	struct Event {
		Event(EventDef *def_, Parameters *params_, std::vector<BaseAction *> &actions_) : def(def_), params(params_) {
	...
	struct Property {
		Property(Location loc_, TokenBase *value_) : loc(loc_), touched(false), value(value_) {}
	...
	void addEvent(EventDef *event, std::unique_ptr<Parameters> &&params, std::vector<BaseAction *> &actions) {
		params->calcCRC(crc);
...
struct EventData {
	...
	EventData(Iter i_, Iter end_) : i(i_), end(end_) {}
...
TokenBase *Parser::getProperty(std::string const &a, std::string const &b) {
	if (TokenBase *p = pimpl->getRawProperty(a))
		return p;
	else if (TokenBase *p = pimpl->getRawProperty(b))
		return p;
	return &globalDefault;
}
