...
class AutoAnnounceServer : public ZCom_Control {
  public:
	...
	~AutoAnnounceServer() {
		delete m_dynamicNode;
		m_dynamicNode = nullptr;
	}
	AutoAnnounceServer(int port)
		: ZCom_Control(), m_connSpawned(0), m_testClass(0), m_nodeRequestCount(0), m_dynamicNode(nullptr) {
  ...
  protected:
	...
	void ZCom_cbNodeRequest_Dynamic(ZCom_ConnID id, uint32_t requested_class, ZCom_BitStream *announcedata, int role,
									uint32_t net_id) override {
		m_nodeRequestCount++;
	}
};
...
class AutoAnnounceClient : public ZCom_Control {
  public:
	...
	AutoAnnounceClient(int port) : ZCom_Control(), m_connected(false), m_testClass(0), m_nodeRequestCount(0) {
		m_testClass = ZCom_registerClass("AutoAnnounceNode", 0);
		g_currentControl = this;
...
class DupServer : public ZCom_Control {
  public:
	...
	DupServer(int port) : ZCom_Control(), m_connSpawned(0), m_testClass(0), m_nodeRequestCount(0), m_node(nullptr) {
...
class DupClient : public ZCom_Control {
  public:
	...
	DupClient(int port) : ZCom_Control(), m_connected(false), m_testClass(0), m_nodeRequestCount(0) {
		m_testClass = ZCom_registerClass("DupNode", 0);
		g_currentControl = this;
		ZCom_initSockets(false, 0, 0, 0);
	}
...
class OwnerAwareServer : public ZCom_Control {
  public:
	...
	OwnerAwareServer(int port)
		: ZCom_Control(), m_connSpawned(0), m_testClass(0), m_client1Role(-1), m_client2Role(-1), m_client1ID(0),
		  m_client2ID(0) {
...
class OwnerAwareClient : public ZCom_Control {
  public:
	...
	OwnerAwareClient(int port)
		: ZCom_Control(), m_connected(false), m_testClass(0), m_receivedRole(-1), m_receivedNodeID(0) {
		m_testClass = ZCom_registerClass("OwnerNode", 0);
		g_currentControl = this;
		ZCom_initSockets(false, 0, 0, 0);
	}
...
class ReannounceServer : public ZCom_Control {
  public:
	...
	ReannounceServer(int port)
		: ZCom_Control(), m_connSpawned(0), m_testClass(0), m_nodeRequestCount(0), m_clientRole(-1), m_clientID(0) {
...
class ReannounceClient : public ZCom_Control {
  public:
	...
	ReannounceClient(int port)
		: ZCom_Control(), m_connected(false), m_testClass(0), m_nodeRequestCount(0), m_receivedRole(-1) {
		m_testClass = ZCom_registerClass("ReannounceNode", 0);
		g_currentControl = this;
		ZCom_initSockets(false, 0, 0, 0);
	}
...
class GetNodeIdServer : public ZCom_Control {
  public:
	...
	GetNodeIdServer(int port)
		: ZCom_Control(), m_connSpawned(0), m_testClass(0), m_announcedNodeId(0), m_clientID(0), m_nodeRequestCount(0),
...
class GetNodeIdClient : public ZCom_Control {
  public:
	...
	GetNodeIdClient(int port)
		: ZCom_Control(), m_connected(false), m_testClass(0), m_nodeRequestCount(0), m_receivedNodeId(0),
		  m_clientNode(nullptr) {
		m_testClass = ZCom_registerClass("GetNodeIdNode", 0);
		g_currentControl = this;
		ZCom_initSockets(false, 0, 0, 0);
...
class TestInterceptor : public ZCom_NodeReplicationInterceptor {
  public:
	...
	TestInterceptor()
		: inPreUpdateCalled(false), inPreUpdateCount(0), inPreUpdateItemCount(0), lastReplicator(nullptr) {}
...
BOOST_AUTO_TEST_CASE(interceptor_in_pre_update_item_receives_estimated_time_sent) {
	...
	struct CaptureEstInterceptor : public ZCom_NodeReplicationInterceptor {
		...
		bool inPreUpdateItem(ZCom_Node *, uint32_t, eZCom_NodeRole, ZCom_Replicator *,
							 uint32_t estimated_time_sent) override {
			gotCalled = true;
...
BOOST_AUTO_TEST_CASE(interceptor_auto_replication_triggered) {
	...
	int32_t testVal = 42;
	node.addReplicationInt(&testVal, 8, false, ZCOM_REPFLAG_MOSTRECENT | ZCOM_REPFLAG_INTERCEPT,
