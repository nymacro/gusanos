...
BOOST_AUTO_TEST_CASE(add_replication_int_delay_overloads) {
	...
	BOOST_CHECK_NO_THROW(
		node.addReplicationInt(&v1, 32, false, ZCOM_REPFLAG_MOSTRECENT, ZCOM_REPRULE_AUTH_2_ALL)); // 5-arg, defaults
	BOOST_CHECK_NO_THROW(node.addReplicationInt(&v2, 16, true, ZCOM_REPFLAG_MOSTRECENT, ZCOM_REPRULE_AUTH_2_ALL,
												0)); // 6-arg, mindelay=0
}
...
BOOST_AUTO_TEST_CASE(add_replication_float_bool_defaults) {
	...
	bool b = false;
	BOOST_CHECK_NO_THROW(
		node.addReplicationFloat(&f, 16, ZCOM_REPFLAG_MOSTRECENT, ZCOM_REPRULE_AUTH_2_ALL)); // 4-arg, defaults
	BOOST_CHECK_NO_THROW(
		node.addReplicationFloat(&f, 16, ZCOM_REPFLAG_MOSTRECENT, ZCOM_REPRULE_AUTH_2_ALL, 50, 200)); // 6-arg
	BOOST_CHECK_NO_THROW(
		node.addReplicationBool(&b, ZCOM_REPFLAG_MOSTRECENT, ZCOM_REPRULE_AUTH_2_ALL)); // 3-arg, defaults
	BOOST_CHECK_NO_THROW(
		node.addReplicationBool(&b, ZCOM_REPFLAG_MOSTRECENT, ZCOM_REPRULE_AUTH_2_ALL, 50, 200)); // 5-arg
}
