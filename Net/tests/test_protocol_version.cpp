...
public:
	...
	VersionServer(int port, int advertiseVersion)
		: m_advertiseVersion(advertiseVersion)
		, m_spawned(0)
		, m_closed(0)
	{
	...
	~VersionServer() { g_currentControl = nullptr; }

	int getProtocolVersion() const override { return m_advertiseVersion; }

protected:
	bool ZCom_cbConnectionRequest(ZCom_ConnID, ZCom_BitStream&, ZCom_BitStream&) override
	{ return true; }
	void ZCom_cbConnectionSpawned(ZCom_ConnID) override { m_spawned++; }
...
public:
	...
	~VersionClient() { g_currentControl = nullptr; }
