...
public:
	...
	~MonSrv() { delete node; node = nullptr; }
	MonSrv(int port) {
	...
	void ZCom_cbConnectionSpawned(uint32_t id) override { spawned++; m_clientConnID = (int)id; }
};
...
public:
	...
	ZCom_Node* node = nullptr;
	~MonCli() { delete node; node = nullptr; }
	MonCli(int port) {
		g_currentControl = this;
	...
	uint32_t ConnectTo(const char* host, int port) {
		...
		a.setAddress(ZCom_Control::eZCom_AddressUDP, 0, hp); return ZCom_Connect(a, nullptr);
	}
...
static int countReceived(MonSrv& srv, MonCli& cli, int ticks) {
	...
	for (int i = 0; i < ticks; i++) {
		...
		if (cli.m_clientVal != last) { received++; last = cli.m_clientVal; }
	}
	return received;
}
...
BOOST_AUTO_TEST_CASE(upstream_limit_does_not_drop_reliable_traffic)
{
	...
	int unlimited = 0;
	{
		MonSrv srv(port); MonCli cli(port);
	...
	int limited = 0;
	{
		MonSrv srv(port + 1); MonCli cli(port + 1);
...
BOOST_AUTO_TEST_CASE(process_replicators_runs_process_callbacks)
{
	...
	for (int i = 0; i < 50; i++) {
		srv.m_serverVal = ++counter;
		g_currentControl = &srv; srv.ZCom_processReplicators(16); g_currentControl = nullptr;
		processBoth(&srv, &cli, 1);
		if (cli.m_clientVal != last) { received++; last = cli.m_clientVal; }
	}
