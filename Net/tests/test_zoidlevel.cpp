...
class ZoidServer : public ZCom_Control {
  public:
	...
	ZoidServer(int udpPort, bool acceptZoid = true)
		: ZCom_Control(), m_connectionSpawnedCount(0), m_zoidRequestReceived(false), m_lastRequestedLevel(0),
...
class ZoidClient : public ZCom_Control {
  public:
	...
	ZoidClient()
		: ZCom_Control(), m_connected(false), m_zoidResultLevel(0), m_zoidResult(eZCom_ZoidDisabled),
  ...
  protected:
	...
	void ZCom_cbZoidResult(ZCom_ConnID id, eZCom_ZoidResult result, uint8_t new_level,
						   ZCom_BitStream &reason) override {
