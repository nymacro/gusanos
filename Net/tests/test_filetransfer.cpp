...
class FileServer : public ZCom_Control {
  public:
	...
	FileServer(int udpPort) : ZCom_Control(), m_spawnedCount(0), m_fileClass(0), m_node(nullptr) {
		m_fileClass = ZCom_registerClass("FileXfer", 0);
		g_currentControl = this;
		ZCom_initSockets(true, udpPort, 0, 0);
	}

	void SendFileChunk(int chunkIdx, int totalChunks, const uint8_t *data, int len) {
		if (!m_node)
			return;
...
class FileClient : public ZCom_Control {
  public:
	...
	FileClient(int udpPort) : ZCom_Control(), m_connected(false), m_fileClass(0) {
		m_fileClass = ZCom_registerClass("FileXfer", 0);
		g_currentControl = this;
...
BOOST_AUTO_TEST_CASE(send_multi_chunk_file) {
...

	{
		...
		for (int i = 0; i < 3; i++) {
			srv.SendFileChunk(i, 3, (const uint8_t *)chunks[i], (int)strlen(chunks[i]));
		}
		g_currentControl = nullptr;
