...
static bool writePatternFile(const char *path, size_t bytes) {
	...
	for (size_t i = 0; i < bytes; ++i)
		buf[i] = static_cast<char>((i * 7 + 3) % 251);
...
BOOST_AUTO_TEST_CASE(full_round_trip) {
	...
	int port = s_fe_port++;
	char srcPath[128];
	snprintf(srcPath, sizeof srcPath, "/tmp/ztest_fe_src_%d.bin", port);
	char dstPath[128];
	snprintf(dstPath, sizeof dstPath, "/tmp/ztest_fe_dst_%d.bin", port);
	std::remove(srcPath);
	std::remove(dstPath);
