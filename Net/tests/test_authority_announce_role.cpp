...
public:
	...
	~AuthRoleServer() { delete m_node; m_node = nullptr; }
	AuthRoleServer(int port)
		: m_testClass(0), m_node(nullptr), m_val(0)
	{
	...
	void createAuthorityNode()
	{
		...
		m_node->addReplicationInt((zS32*)&m_val, 32, false,
			ZCOM_REPFLAG_MOSTRECENT, ZCOM_REPRULE_NONE, 0);
...
public:
	...
	~AuthRoleClient() { delete m_node; m_node = nullptr; }
	AuthRoleClient()
		: m_connected(false), m_testClass(0), m_receivedRole(-1)
		, m_receivedNodeID(0), m_node(nullptr), m_val(0)
	{
		m_testClass = ZCom_registerClass("AuthRoleNode", 0);
		g_currentControl = this;
...
protected:
	...
	void ZCom_cbNodeRequest_Dynamic(ZCom_ConnID, uint32_t, ZCom_BitStream*,
	                               int role, uint32_t net_id) override
	{
		m_receivedRole = role;
