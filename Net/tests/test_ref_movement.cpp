...
BOOST_AUTO_TEST_CASE(movement_replicator_creation) {
	...
	ZCom_Replicate_Movement<zFloat, 3> *move = new ZCom_Replicate_Movement<zFloat, 3>(setup);
	BOOST_REQUIRE(move);
...
class TestMoveListener : public ZCom_MoveUpdateListener<zFloat, 3> {
  public:
	...
	TestMoveListener()
		: input_updated_called(false), input_sent_called(false), correction_called(false),
		  update_received_called(false) {}

	void inputUpdated(ZCom_BitStream &_inputstream, bool _inputchanged, uint32_t _client_time,
					  uint32_t _estimated_time_sent) override {
		(void)_inputstream;
		(void)_inputchanged;
		(void)_client_time;
	...
	void correctionReceived(zFloat *_pos, zFloat *_vel, zFloat *_acc, bool _teleport,
							uint32_t _estimated_time_sent) override {
		(void)_pos;
		(void)_vel;
		(void)_acc;
	...
	void updateReceived(ZCom_BitStream &_inputstream, zFloat *_pos, zFloat *_vel, zFloat *_acc,
						uint32_t _estimated_time_sent) override {
		(void)_inputstream;
		(void)_pos;
		(void)_vel;
