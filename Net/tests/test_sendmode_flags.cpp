...
BOOST_AUTO_TEST_CASE(reliable_ordered_maps_to_reliable) {
	BOOST_CHECK_EQUAL(enetPacketFlags(eZCom_ReliableOrdered), (enet_uint32)ENET_PACKET_FLAG_RELIABLE);
}

BOOST_AUTO_TEST_CASE(reliable_maps_to_reliable) {
	BOOST_CHECK_EQUAL(enetPacketFlags(eZCom_Reliable), (enet_uint32)ENET_PACKET_FLAG_RELIABLE);
}
