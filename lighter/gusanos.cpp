		...
		for (int x = x1; x <= x2; ++x) {
			...
			switch (myDir) {
				case 0:
					break; // Do nothing
				case 1:
					add *= double(abs(lightX - x)) / l;
					break;
				case 2:
					add *= double(abs(lightX - x)) / l;
					break;
				case 3:
					add *= double(abs(lightY - y)) / l;
					break;
				case 4:
					add *= double(abs(lightY - y)) / l;
					break;
				...
				for (int n = 0; n < (int)lightSources.size(); ++n) {
					if (true) // rnd() < prob )
					{
						...
						if (!level.preciseTrace(lightSources[n].x + 0.5f, lightSources[n].y + 0.5f, x, y, pred)) {
							...
							if (tmpDist < minDistanceSqr || minDistanceSqr < 0)
								minDistanceSqr = tmpDist;
							color += 255;
						}
					}
				}
