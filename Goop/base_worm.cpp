...
BaseWorm::BaseWorm()
	: BaseObject(), aimSpeed(0.0), aimAngle(90.0), currentWeapon(0), m_weaponCount(0), m_lastHurt(0)
#ifndef DEDSERV
	  ,
	  m_animator(0)
#endif
...
void BaseWorm::calculateAllReactionForces(BaseVec<float> &nextPos, BaseVec<long> &inextPos) {
	...
	if (reacts[Down] < 2 && reacts[Up] > 0 && (reacts[Left] > 0 || reacts[Right] > 0)) {
	...
	if (reacts[Up] < 2 && reacts[Down] > 0 && (reacts[Left] > 0 || reacts[Right] > 0)) {
	...
	if (game.options.worm_disableWallHugging) {
		if (reacts[Up] == 1 && reacts[Down] == 1 && (reacts[Left] > 0 || reacts[Right] > 0)) {
			reacts[Up] = 0;
			reacts[Down] = 0;
		}
	}
	...
	if (m_isActive) {
...

		{
			...
			if (Weapon *w = getCurrentWeapon())
				w->drawBottom(where, renderX, renderY);
			...
			if (Weapon *w = getCurrentWeapon())
				w->drawTop(where, renderX, renderY);

			if (m_currentFirecone) {
				Vec distance = Vec(aimAngle, (double)m_fireconeDistance);
				m_currentFirecone->getSprite(m_fireconeAnimator->getFrame(), getAngle())
					->draw(where, renderX + static_cast<int>(distance.x) * m_dir,
						   renderY + static_cast<int>(distance.y));
			}
...
void BaseWorm::die() {
	std::string weaponName = (m_lastHurtWeapon >= 0 && (size_t)m_lastHurtWeapon < game.weaponList.size())
	...
	if (!self) {
		...
		if (m_lastHurt && m_lastHurt->stats)
			atk = m_lastHurt->stats.get();
		else if (m_lastHurtShooterID)
			atk = Network::findSavedStats(m_lastHurtShooterID);
		if (atk) {
			atk->kills++;
...
void BaseWorm::damage(float amount, BasePlayer *damager, DamageCause const &cause) {
	...
	bool self = (damager && damager == m_owner) || (m_owner && cause.shooterID == m_owner->getOptions()->uniqueID);
	if (!self && effective > 0) {
		if (m_owner && m_owner->stats)
			m_owner->stats->damageTaken += effective;
		BasePlayer::Stats *atk = 0;
		if (damager && damager->stats)
			atk = damager->stats.get();
		else if (cause.shooterID)
			atk = Network::findSavedStats(cause.shooterID);
		if (atk)
			atk->damageDealt += effective;
	}
}
