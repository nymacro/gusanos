...
*/
LMETHOD(LuaSocket, tcp_send, size_t len; char const *s = lua_tolstring(context, 2, &len); if (!s) return 0;
		p->send(s, len); return 0;)
...
*/
LMETHOD(LuaSocket, tcp_think, p->think(); if (p->begin() == p->end()) return 0;
		DLOG((void *)p->begin() << ", " << (void *)p->end() << ", size: " << (p->end() - p->begin()));
		lua_pushlstring(context, p->begin(), p->end() - p->begin()); return 1;)

LMETHOD(LuaSocket, tcp_destroy, p->~LuaSocket(); return 0;)
...
*/
METHODC(
	ZCom_BitStream, bitStream_encodeEliasGamma, int v = lua_tointeger(context, 2); if (v < 1) {
		WLOG("Number " << v << " can't be encoded with elias gamma. Encoded as 1.");
		v = 1;
	} Encoding::encodeEliasGamma(*p, static_cast<unsigned int>(v));
	context.pushvalue(1); return 1;)

/*! Bitstream:decode_elias_gamma()

	Extracts an integer above or equal to 1 encoded using the elias gamma universal encoding.
*/
METHODC(ZCom_BitStream, bitStream_decodeEliasGamma, context.push(Encoding::decodeEliasGamma(*p)); return 1;)

/*! Bitstream:add_bool()

	Extracts a boolean value from this bitstream (Returns either true or false).
*/
METHODC(ZCom_BitStream, bitStream_addBool, p->addBool(lua_toboolean(context, 2)); context.pushvalue(1); return 1;)

/*! Bitstream:get_bool()

	Extracts a boolean value from this bitstream (Returns either true or false).
*/
METHODC(ZCom_BitStream, bitStream_getBool, context.push(p->getBool()); return 1;)
...
*/
METHODC(ZCom_BitStream, bitStream_addInt, int bits = 32; if (lua_gettop(context) >= 3) bits = lua_tointeger(context, 3);
		p->addInt(lua_tointeger(context, 2), bits); context.pushvalue(1); return 1;)
...
/*! Bitstream:get_int([bits])
	...
	If bits is left out, 32 bits is assumed.
*/
METHODC(ZCom_BitStream, bitStream_getInt, int bits = 32; if (lua_gettop(context) >= 2) bits = lua_tointeger(context, 2);
		context.push(static_cast<int>(p->getInt(bits))); return 1;)

/*! Bitstream:add_string(str)

	Adds the string //str// to this bitstream.
*/
METHODC(ZCom_BitStream, bitStream_addString, p->addString(lua_tostring(context, 2)); context.pushvalue(1); return 1;)

/*! Bitstream:get_string()

	Extracts a string from this bitstream.
*/
METHODC(ZCom_BitStream, bitStream_getString, context.push(p->getStringStatic()); return 1;)

METHODC(ZCom_BitStream, bitStream_destroy, delete p; return 0;)
...
#define LUA_EVENT_SEND_METHOD(type_, params_, decl_, cases_, body_)                                                    \
	LMETHOD(                                                                                                           \
		LuaEventDef, luaEvent_##type_##_send, if (p->idx > 0) {                                                        \
			...
			decl_ switch (lua_gettop(context)) {                                                                       \
				default: {                                                                                             \
					if (lua_gettop(context) < params_ + 3)                                                             \
						break;                                                                                         \
					[[fallthrough]];                                                                                   \
				}                                                                                                      \
				case params_ + 5:                                                                                      \
					rules = lua_tointeger(context, params_ + 5);                                                       \
					[[fallthrough]];                                                                                   \
				case params_ + 4:                                                                                      \
					mode = (eZCom_SendMode)lua_tointeger(context, params_ + 4);                                        \
					[[fallthrough]];                                                                                   \
				case params_ + 3:                                                                                      \
					connID = (ZCom_ConnID)lua_tointeger(context, params_ + 3);                                         \
					[[fallthrough]];                                                                                   \
				case params_ + 2:                                                                                      \
					userdata = ASSERT_OBJECT(ZCom_BitStream, params_ + 2);                                             \
					cases_                                                                                             \
			}                                                                                                          \
			body_                                                                                                      \
		} return 0;)
...
*/
LUA_EVENT_SEND_METHOD(player, 1, BasePlayer *player = 0;, case 2 : player = ASSERT_OBJECT(BasePlayer, 2);
					  , if (player) player->sendLuaEvent(p, mode, rules, userdata, connID);)
...
void initNetwork(LuaContext &context) {
	context.functions()("tcp_connect", l_tcp_connect)("network_game_event", l_network_game_event)(
		"network_player_event", l_network_player_event)("network_worm_event", l_network_worm_event)(
		"network_particle_event", l_network_particle_event)("new_bitstream", l_new_bitstream);

	CLASSM(Socket, ("__gc", l_tcp_destroy), ("send", l_tcp_send)("think", l_tcp_think))

	CLASS(LuaGameEvent, ("send", l_luaEvent_game_send))

	CLASS(LuaPlayerEvent, ("send", l_luaEvent_player_send))

	CLASS(LuaWormEvent, ("send", l_luaEvent_worm_send))

	CLASS(LuaParticleEvent, ("send", l_luaEvent_particle_send))

	CLASSM(ZCom_BitStream, ("__gc", l_bitStream_destroy),
		   ("add_int", l_bitStream_addInt)("get_int", l_bitStream_getInt)("add_string", l_bitStream_addString)(
			   "get_string", l_bitStream_getString)("encode_elias_gamma", l_bitStream_encodeEliasGamma)(
			   "decode_elias_gamma", l_bitStream_decodeEliasGamma)("add_bool", l_bitStream_addBool)(
			   "get_bool", l_bitStream_getBool)("dump", l_bitStream_dump)("undump", l_bitStream_undump))

	ENUM(SendMode, ("ReliableUnordered",
					eZCom_ReliableUnordered)("ReliableOrdered", eZCom_ReliableOrdered)("Unreliable", eZCom_Unreliable))

	ENUM(RepRule, ("Auth2All", ZCOM_REPRULE_AUTH_2_ALL)("Auth2Owner", ZCOM_REPRULE_AUTH_2_OWNER)(
					  "Auth2Proxy", ZCOM_REPRULE_AUTH_2_PROXY)("None", ZCOM_REPRULE_NONE)("Owner2Auth",
																						  ZCOM_REPRULE_OWNER_2_AUTH))

	ENUM(Network, ("Connecting", Network::StateConnecting)("Disconnecting", Network::StateDisconnecting)(
					  "Disconnected", Network::StateDisconnected)("Hosting", Network::StateHosting))
