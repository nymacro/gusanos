	...
	*/
	...
	switch (params) {
		default:
			...
			[[fallthrough]];
		case 10:
			distanceOffset = lua_tonumber(L, 10); /* fall through */
		case 9:
			angleOffset = AngleDiff(lua_tonumber(L, 9)); /* fall through */
		case 8:
			distribution = AngleDiff(lua_tonumber(L, 8)); /* fall through */
		case 7:
			amountVariation = lua_tointeger(L, 7); /* fall through */
		case 6:
			motionInheritance = lua_tonumber(L, 6); /* fall through */
		case 5:
			speedVariation = lua_tonumber(L, 5); /* fall through */
		case 4:
			speed = lua_tonumber(L, 4); /* fall through */
		case 3:
			amount = lua_tointeger(L, 3);
	}
...
*/
METHODC(BaseWorm, worm_getHealth, context.push(p->getHealth()); return 1;)
...
METHODC(BaseWorm, worm_isChanging, context.push(p->isChanging()); return 1;)
...
METHODC(BaseObject, baseObject_getAngle, lua_pushnumber(context, p->getAngle().toDeg()); return 1;)
...
*/
METHODC(BaseObject, baseObject_remove, p->deleteMe = true; return 0;)
...
METHODC(BaseObject, baseObject_pos, context.push(p->pos.x); context.push(p->pos.y); return 2;)
...
*/
METHODC(BaseObject, baseObject_setPos, p->setPos(Vec(lua_tonumber(context, 2), lua_tonumber(context, 3))); return 0;)
...
*/
METHODC(BaseObject, baseObject_push, p->spd.x += lua_tonumber(context, 2); p->spd.y += lua_tonumber(context, 3);
		return 0;)
...
METHODC(
	BaseObject, baseObject_data,
	if (p->luaData) { lua.pushReference(p->luaData); } else {
...
*/
METHODC(BaseObject, baseObject_getPlayer, if (!p->getOwner()) return 0;
		lua.pushReference(p->getOwner()->getLuaReference()); return 1;)
...
*/
METHODC(
	BaseObject, baseObject_getClosestWorm,

	Vec from = p->pos;

	int fromx = int(from.x); int fromy = int(from.y);

	BaseWorm *minWorm = 0; float minDistSqr = 10000000.f;

	for (std::list<BasePlayer *>::iterator playerIter = game.players.begin(); playerIter != game.players.end();
		 ++playerIter) {
...
/*! Object:shoot(type, amount, speed, speedVariation, motionInheritance, amountVariation, distribution, angleOffset,
   distanceOffset)
...
METHODC(
	Particle, particle_set_replication,
	...
	if (lua_toboolean(context, 3)) p->setFlag(mask);
	else p->resetFlag(mask);
...
METHODC(Weapon, weaponinst_reloading, context.push(p->reloading); return 1;)
...
*/
METHODC(Weapon, weaponinst_reload_time, context.push(p->getReloadTime()); return 1;)

/*! Weapon:ammo()

	Returns the amount of ammo left in this weapon.
*/
METHODC(Weapon, weaponinst_ammo, context.push(p->getAmmo()); return 1;)
...
void addBaseObjectFunctions(LuaContext &context) {
	context.tableFunctions()("remove", l_baseObject_remove)("pos", l_baseObject_pos)("spd", l_baseObject_spd)(
		"push", l_baseObject_push)
#ifndef NO_DEPRECATED
		("get_player", l_baseObject_getPlayer_depr)("get_closest_worm", l_baseObject_getClosestWorm_depr)(
			"get_angle", l_baseObject_getAngle_depr)
#endif
			("player", l_baseObject_getPlayer)("closest_worm", l_baseObject_getClosestWorm)("data", l_baseObject_data)(
				"shoot", l_baseObject_shoot)("angle", l_baseObject_getAngle)("set_pos", l_baseObject_setPos)(
				"set_spd", l_baseObject_setSpd)("damage", l_baseObject_damage);
}
...
void initObjects() {
	...
	lua_newtable(context);
	context.tableFunctions()("__gc", l_particle_destroy);
	lua_pushstring(context, "__index");
	...
	context.tableFunctions()("set_angle", l_particle_setAngle)("set_replication", l_particle_set_replication);
	...
	lua_newtable(context);
	context.tableFunctions()("__gc", l_worm_destroy);
	lua_pushstring(context, "__index");
...
#endif
			("health", l_worm_getHealth)("current_weapon", l_worm_current_weapon)("is_changing", l_worm_isChanging);
	...
	CLASSM_(Weapon, ("__gc", l_weaponinst_destroy),
			("is_reloading", l_weaponinst_reloading)("reload_time", l_weaponinst_reload_time)(
				"ammo", l_weaponinst_ammo)("type", l_weaponinst_type))

	ENUM(Particle, ("Position", ParticleRep::Position))
}
