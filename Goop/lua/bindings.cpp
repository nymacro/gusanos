...
int l_console_register_command(lua_State *L) {
	...
	console.registerCommands()(name, boost::bind(LuaBindings::luaConsoleCommand, ref, _1), true);
...
int l_fetch_server_list(lua_State *L) {
	...
	network.addHttpRequest(std::move(req), boost::bind(serverListCallb, L, ref, _1));
