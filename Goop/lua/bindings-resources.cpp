...
METHODC(
	...
	{
		LUA_WLOG_ONCE("The r, g, b version of render_skinned_box is deprecated, replace the r, g, b parameters by "
					  "color(r, g, b)");
...
*/
METHODC(SpriteSet, sprites_frames, context.push(static_cast<int>(p->getFramesWidth())); return 1;)
...
METHODC(
	...
	int x = lua_tointeger(context, 4); int y = lua_tointeger(context, 5);

	int cr = 255; int cg = 255; int cb = 255; int flags = 0;

	if (params >= 6) {
		int c = lua_tointeger(context, 6);
		if (params >= 8) {
			LUA_WLOG_ONCE(
				"The r, g, b version of Font:render is deprecated, replace the r, g, b parameters by color(r, g, b)");
			cr = c;
	...
	if (flags & Shadow) realFlags |= Font::Shadow; if (flags & Formatting) realFlags |= Font::Formatting;
...
*/
METHODC(
	Sound, sound_play, lua_Number loudness = 100.0; lua_Number pitch = 1.0; lua_Number pitchVariation = 1.0;
	...
	if (BaseObject *obj = getObject<BaseObject>(context, 2)) {
		...
		switch (params) {
			default:
				...
				[[fallthrough]];
			case 5:
				pitchVariation = lua_tonumber(context, 6); /* fall through */
			case 4:
				pitch = lua_tonumber(context, 5); /* fall through */
			case 3:
				loudness = lua_tonumber(context, 4); /* fall through */
			case 2:
				break;
		}
		...
		switch (params) {
			default:
				...
				[[fallthrough]];
			case 6:
				pitchVariation = lua_tonumber(context, 6);
				[[fallthrough]];
			case 5:
				pitch = lua_tonumber(context, 5);
				[[fallthrough]];
			case 4:
				loudness = lua_tonumber(context, 4);
				[[fallthrough]];
			case 3:
				break;
		}
...
*/
METHODC(WeaponType, weapon_next, size_t n = p->getIndex() + 1; if (n >= game.weaponList.size()) n = 0;
		game.weaponList[n]->pushLuaReference(); return 1;)

/*! WeaponType:prev()

	Returns the previous weapon type after this one.
*/
METHODC(WeaponType, weapon_prev, size_t n = p->getIndex(); if (n == 0) n = game.weaponList.size() - 1; else --n;
		game.weaponList[n]->pushLuaReference(); return 1;)
...
*/
METHODC(WeaponType, weapon_name, lua_pushstring(context, p->name.c_str()); return 1;)

/*! WeaponType:reload_time()

	Returns the time it takes for this weapon type to reload.
*/
METHODC(WeaponType, weapon_reload_time, context.push(p->reloadTime); return 1;)
...
METHODC(
	PartType, parttype_put, float x = 0.f; float y = 0.f; float xspd = 0.f; float yspd = 0.f; Angle angle(0.0);

	int params = lua_gettop(context); switch (params) {
		default:
			if (params < 3)
				return 0;
			[[fallthrough]];
		case 6:
			angle = Angle(lua_tonumber(context, 6));
			[[fallthrough]];
		case 5:
			yspd = lua_tonumber(context, 5);
			[[fallthrough]];
		case 4:
			xspd = lua_tonumber(context, 4);
			[[fallthrough]];
		case 3:
			y = lua_tonumber(context, 3);
			[[fallthrough]];
		case 2:
			x = lua_tonumber(context, 2);
	}
...
METHOD(PartType, parttype_destroy, if (!p) return 0; delete p; return 0;)
...
void initResources() {
	...
	context.functions()("load_particle", l_load_particle)("weapon_random", l_weapon_random)("weapon_count",
																							l_weapon_count)
#ifndef DEDSERV
		("sprites_load", l_sprites_load)("font_load", l_font_load)
#endif
			("map_is_loaded", l_map_is_loaded)("maps", l_maps)("mods", l_mods);

	CLASSM_(PartType, ("__gc", l_parttype_destroy), ("put", l_parttype_put))

	CLASSM_(WeaponType, ("__gc", l_weapon_destroy),
			("next", l_weapon_next)("prev", l_weapon_prev)("name", l_weapon_name)("reload_time", l_weapon_reload_time)(
				"ammo", l_weapon_ammo))

#ifndef DEDSERV
	// Font method and metatable

	CLASS(Font, ("render", l_font_render))

	ENUM(Font, ("None", None)("CenterV", CenterV)("CenterH", CenterH)("Right", Right)("Bottom", Bottom)(
				   "Shadow", Shadow)("Formatting", Formatting))

	CLASS(Sound, ("play", l_sound_play))

	CLASS(SpriteSet,
		  ("render", l_sprites_render)("render_skinned_box", l_sprites_render_skinned_box)("frames", l_sprites_frames))
