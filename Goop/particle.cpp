...
class ParticleInterceptor : public ZCom_NodeReplicationInterceptor {
  public:
	...
	ParticleInterceptor(Particle *parent_) : parent(parent_) {}
...
#endif
	  ,
	  m_origin(pos_), m_node(0), interceptor(0) {
	m_type->touch();

	m_angle.clamp();

	flags = (dir > 0 ? FaceRight : 0) | RepPos;
...
void Particle::assignNetworkRole(bool authority) {
	...
	static ZCom_ReplicatorSetup posSetup(ZCOM_REPFLAG_MOSTRECENT | ZCOM_REPFLAG_INTERCEPT, ZCOM_REPRULE_AUTH_2_ALL,
										 ParticleInterceptor::Position, -1, 1000);
...
void Particle::sendLuaEvent(LuaEventDef *event, eZCom_SendMode mode, zU8 rules, ZCom_BitStream *userdata,
							ZCom_ConnID connID) {
	if (!m_node)
		return;
	ZCom_BitStream data;
