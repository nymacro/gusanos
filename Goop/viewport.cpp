...
Viewport::Viewport() : dest(0), hud(0) {
...
struct TestCuller : public Culler<TestCuller> {
	TestCuller(BITMAP *dest_, BITMAP *src_, int scrOffX_, int scrOffY_, int destOffX_, int destOffY_, Rect const &rect)
		: Culler<TestCuller>(rect), dest(dest_), src(src_), scrOffX(scrOffX_), scrOffY(scrOffY_), destOffX(destOffX_),
		  destOffY(destOffY_) {}
	...
	void line(int y, int x1, int x2) {
		...
		drawSpriteLine_add(dest, src, x1 + scrOffX, y + scrOffY, x1 + destOffX, y + destOffY, x2 + destOffX + 1, 255);
	}
...
void Viewport::setDestination(BITMAP *where, int x, int y, int width, int height) {
	if (width > where->w || height > where->h)
		return;
