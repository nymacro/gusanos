...
BasePlayer::BasePlayer(boost::shared_ptr<PlayerOptions> options, BaseWorm *worm)
	: stats(new Stats), deleteMe(false), colour(options->colour), team(options->team), local(false), luaData(0),
	  m_worm(0), m_options(options), m_isAuthority(false), m_processingNetworkEvent(false), m_lastMoveQuant(),
	  m_node(0), m_interceptor(0), m_wormID(INVALID_NODE_ID),
...
void BasePlayer::deleteThis() {
	...
	if (luaReference) {
		...
		// metamethod (player_destroy) will not delete this object again
		// after the C++ side has already freed it.
...
#endif
						std::cout << "[DEBUG] decoded event=" << (int)event << " datalen=" << data->getDataLength()
								  << " writeBit=" << data->getBitLength()
								  << " bitsOfEventCount=" << Encoding::bitsOf(EVENT_COUNT - 1) << std::endl;
						switch (event) {
							case ACTION_START: // ACTION TART LOL TBH
							{
								...
								std::cout << "[DEBUG] BasePlayer::think dispatching action=" << (int)action
										  << " (event=" << (int)event << ")" << std::endl;
							...
							case LuaEvent: {
								...
								if (LuaEventDef *event =
										network.indexToLuaEvent(Network::LuaEventGroup::Player, index)) {
...
void BasePlayer::assignNetworkRole(bool authority, ZCom_BitStream *announceData, ZCom_NodeID net_id) {
	...
	m_node->addReplicationInt((zS32 *)&m_wormID, 32, false, ZCOM_REPFLAG_MOSTRECENT | ZCOM_REPFLAG_INTERCEPT,
...
bool BasePlayerInterceptor::inPreUpdateItem(ZCom_Node *_node, ZCom_ConnID _from, eZCom_NodeRole _remote_role,
	...
	switch (action) {
		case LEFT: {
			...
			if (m_node) {
				...
				if (q != m_lastMoveQuant[0] && (q == 0 || std::abs(q - m_lastMoveQuant[0]) > 1)) {
					...
					m_node->sendEvent(eZCom_ReliableOrdered, ZCOM_REPRULE_AUTH_2_PROXY | ZCOM_REPRULE_OWNER_2_AUTH,
									  &data);
				}
			}
		} break;
		...
		case RESPAWN: {
			if (m_worm && !m_worm->isActive()) {
				std::cout << "[DEBUG] baseActionStart RESPAWN: m_worm=" << (void *)m_worm
						  << " isActive=" << (m_worm ? m_worm->isActive() : false) << " m_node=" << (void *)m_node
						  << std::endl;
				m_worm->respawn();
			}
			if (m_node) {
