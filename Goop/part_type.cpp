...
BaseObject *newParticle_Particle(PartType *type, Vec pos_ = Vec(0.f, 0.f), Vec spd_ = Vec(0.f, 0.f), int dir = 1,
								 BasePlayer *owner = NULL, Angle angle = Angle(0)) {
	if (type->needsNode && network.isClient())
		return 0;
...
#endif
		|| damping != 1.f || acceleration != 0.f || !groundCollision || death || timer.size() > 1 || line2Origin ||
		detectRanges.size() > 0 || needsNode) {
		return false;
	}
...
bool PartType::load(fs::path const &filename) {
	...
	parser.addEvent("timer", EventID::Timer, af::Object)("delay")("delay_var")("max_trigger");

	parser.addEvent("custom_event", EventID::CustomEvent, af::Object | af::Object2)("index", false);

	parser.addEvent("detect_range", EventID::DetectRange, af::Object | af::Object2)("range")("detect_owner")("layers");
	...
	if (animtypestr == "ping_pong")
		animType = ANIM_PINGPONG;
	else if (animtypestr == "loop_right")
		animType = ANIM_LOOPRIGHT;
	else if (animtypestr == "right_once")
		animType = ANIM_RIGHTONCE;
	...
	for (; i; ++i) {
		...
		switch (i.type()) {
			...
			case EventID::DetectRange: {
				...
				detectRanges.push_back(
					new DetectEvent(i.actions(), p[0]->toDouble(10.0), p[1]->toBool(true), detectFilter));
			} break;
