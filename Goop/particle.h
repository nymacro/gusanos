...
class Particle : public BaseObject {
  public:
	...
	enum Flags { FaceRight = (1 << 0), RepPos = (1 << 1) };

	static ZCom_ClassID classID;

	Particle(PartType *type, Vec pos_ = Vec(0.f, 0.f), Vec spd_ = Vec(0.f, 0.f), int dir = 1, BasePlayer *owner = NULL,
			 Angle angle = Angle(0));
	~Particle();
