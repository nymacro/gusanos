...
void Options::registerInConsole() {
	console.registerVariables()("SV_NINJAROPE_SHOOT_SPEED", &ninja_rope_shootSpeed, 2)("SV_NINJAROPE_PULL_FORCE",
																					   &ninja_rope_pullForce, 0.031)
		//("SV_NINJAROPE_PULL_FORCE", &ninja_rope_pullForce, (4.0f / 3.0f) * (70.0f * 70.0f / (100.0f * 100.0f
		//* 16.0f)))

		//("SV_NINJAROPE_START_DISTANCE", &ninja_rope_startDistance, 20)
		("SV_NINJAROPE_START_DISTANCE", &ninja_rope_startDistance, 4000.0f / 16.f - 1.f)("SV_NINJAROPE_MAX_LENGTH",
																						 &ninja_rope_maxLength, 2000.f)

			("SV_WORM_MAX_SPEED", &worm_maxSpeed, 0.45)("SV_WORM_ACCELERATION", &worm_acceleration, 0.03)(
				"SV_WORM_AIR_ACCELERATION_FACTOR", &worm_airAccelerationFactor, 1.f)
		//("SV_WORM_FRICTION", &worm_friction, 0.02)
		("SV_WORM_FRICTION", &worm_friction, pow(0.89, 0.7))("SV_WORM_AIR_FRICTION", &worm_airFriction, 1.f)(
			"SV_WORM_GRAVITY", &worm_gravity, 0.009)("SV_WORM_DISABLE_WALL_HUGGING", &worm_disableWallHugging, 0)
		//("SV_WORM_BOUNCE_QUOTIENT", &worm_bounceQuotient, 0.3)
		("SV_WORM_BOUNCE_QUOTIENT", &worm_bounceQuotient, 0.333)("SV_WORM_BOUNCE_LIMIT", &worm_bounceLimit, 2)
		//("SV_WORM_BOUNCE_LIMIT", &worm_bounceLimit, 0.56875f)

		("SV_WORM_JUMP_FORCE", &worm_jumpForce, 0.6)("SV_WORM_WEAPON_HEIGHT", &worm_weaponHeight, 5)(
			"SV_WORM_HEIGHT", &worm_height, 9)("SV_WORM_WIDTH", &worm_width, 3)("SV_WORM_MAX_CLIMB", &worm_maxClimb, 4)(
			"SV_WORM_BOX_RADIUS", &worm_boxRadius, 2)("SV_WORM_BOX_TOP", &worm_boxTop, 3)("SV_WORM_BOX_BOTTOM",
																						  &worm_boxBottom, 4)

			("SV_MAX_RESPAWN_TIME", &maxRespawnTime, -1)("SV_MIN_RESPAWN_TIME", &minRespawnTime, 100)("SV_TEAM_PLAY",
																									  &teamPlay, 0)

				("HOST", &host, 0)

					("SV_MAX_WEAPONS", &maxWeaponsVar, 5)("CL_SPLITSCREEN", &splitScreenVar, 0)

						("RCON_PASSWORD", &rConPassword, "")("CL_SHOW_MAP_DEBUG", &showMapDebug, 0)(
							"CL_SHOW_DEATH_MESSAGES", &showDeathMessages, true)("CL_LOG_DEATH_MESSAGES",
...
void Game::think() {
	...
	mq_process_messages(msg) mq_case(ChangeLevel)
	...
	mq_end_case() mq_end_process_messages()
												...
												case RegisterLuaEvents:
												{
														...
														t < Network::LuaEventGroup::Max; ++t)
													{
														...
														for(int i = 0; i < c; ++i)
														{
															...
															network.indexLuaEvent((Network::LuaEventGroup::type)t,
							   name);
														}
													}
												}
...
void Game::assignNetworkRole(bool authority) {
	...
	m_node->addReplicationInt((zS32 *)&options.worm_gravity, 32, false,
							  ZCOM_REPFLAG_MOSTRECENT | ZCOM_REPFLAG_RARELYCHANGED, ZCOM_REPRULE_AUTH_2_ALL);
	m_node->addReplicationInt((zS32 *)&options.teamPlay, 1, false, ZCOM_REPFLAG_MOSTRECENT | ZCOM_REPFLAG_RARELYCHANGED,
...
void Game::displayKillMsg(BasePlayer *killed, BasePlayer *killer, std::string const &weaponName,
	...
	if (options.showDeathMessages)
		displayMessage(ScreenMessage(ScreenMessage::Death, str, 400));
	if (options.logDeathMessages)
		console.addLogMsg(str);
}
...
bool Game::checkCRCs(ZCom_BitStream &data) {
	...
	if (particleCRC != partTypeList.crc() || expCRC != expTypeList.crc() || weaponCRC != getWeaponCRC() ||
