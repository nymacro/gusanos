...
mq_define_message(Connect, 0, (std::string addr_)) : addr(addr_) {}
...
void registerToMasterServer() {
	...
	push_back(data)("action", "add")("title",
									 serverName)("desc", serverDesc)("port", convert<std::string>::value(m_serverPort))(
		"protocol",
		convert<std::string>::value(Network::protocolVersion))("mod", game.getModName())("map", game.level.getName());
...
Network::Network() : clientRetry(false) {
	/*
		m_host = false;
		m_zcom = NULL;
		m_control = NULL;
...
void Network::update() {
	...
	switch (state) {
		...
		case StateIdle: {
			mq_process_messages(msg) mq_case(Connect) if (!isDisconnected()) {
				// if(!isDisconnecting())
				disconnect();
				...
				*/
				...
				if (registerGlobally && ++updateTimer > 6000 * 3) {
					...
					if (!serverAdded) {
						...
						std::list<std::pair<std::string, std::string>> data;
						push_back(data)("action", "update")("port", convert<std::string>::value(m_serverPort));
...
std::unique_ptr<HTTP::Request> Network::fetchServerList() {
	std::list<std::pair<std::string, std::string>> data;
	push_back(data)("action", "list")("protocol", convert<std::string>::value(protocolVersion));
...
void Network::disconnect(DConnEvents event) {
	...
	if (serverAdded) {
		std::list<std::pair<std::string, std::string>> data;
		push_back(data)("action", "remove")("port", convert<std::string>::value(m_serverPort));
