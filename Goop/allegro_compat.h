...
inline int get_color_depth() {
	extern int current_color_depth;
	return current_color_depth;
} // Defaulting to 32

// 16-bit color functions (5-6-5 RGB format)
inline int getr16(int c) {
	return ((c >> 11) & 0x1F) * 255 / 31;
}
inline int getg16(int c) {
	return ((c >> 5) & 0x3F) * 255 / 63;
}
inline int getb16(int c) {
	return (c & 0x1F) * 255 / 31;
}
inline int makecol16(int r, int g, int b) {
...
inline void set_color_depth(int depth) {
	extern int current_color_depth;
	current_color_depth = depth;
}
inline int get_color_conversion() {
	extern int current_color_conversion;
	return current_color_conversion;
}
...
inline int makecol_depth(int depth, int r, int g, int b) {
...
inline void circle(BITMAP *bmp, int x, int y, int radius, int color) {
	_circle(bmp, x, y, radius, color, false);
}
inline void circlefill(BITMAP *bmp, int x, int y, int radius, int color) {
	_circle(bmp, x, y, radius, color, true);
}
