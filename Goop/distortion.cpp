	...
	for (int y = 0; y < radius * 2; ++y)
		for (int x = 0; x < radius * 2; ++x) {
			...
			if (delta.length() <= radius) {
				if (delta.length() != 0) {
					float newLength =
						(radius - sqrt(radius * radius - delta.length() * delta.length())) - delta.length();
...
Distortion::Distortion(DistortionMap *map) : m_map(map) {
	m_map->compileMap();
	width = m_map->width;
...
#define WORK_GENERAL()                                                                                                 \
	for (int y = y1; h-- > 0; ++y, ++desty) {                                                                          \
		...
		switch (bitmap_color_depth(buffer)) {
			case 32:
				WORK(GET32, SET32, Pixel32);
				break;
			case 16:
				WORK(GET16, SET16, Pixel16);
				break;
			default:
				WORK_GENERAL();
				break;
