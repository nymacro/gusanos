...
inline int scaleColor(int a, int b, int bmax) {
	return makecol(getr(a) * b / bmax, getg(a) * b / bmax, getb(a) * b / bmax);
}

inline int brightenColor(int a, int b) {
	return makecol(std::min(getr(a) + b, 255), std::min(getg(a) + b, 255), std::min(getb(a) + b, 255));
}

#ifndef DEDSERV

Sprite::Sprite(Sprite const &b, Sprite const &mask, int color)
	: m_bitmap(create_bitmap_ex(bitmap_color_depth(b.m_bitmap), b.m_bitmap->w, b.m_bitmap->h)) /*, m_mirror(0)*/,
...
void Sprite::drawCut(BITMAP *where, int x, int y, BlitterContext const &blender, int alignment, int left, int top,
