...
union mmx_reg {
	...
	mmx_reg(unsigned long long v) : uq(v) {}
...
#define mmx_mr(op, mem, reg) __asm__ __volatile__(#op " %%" #reg ", %0" : "=m"(mem) : /* nothing */)

#define mmx_rr(op, regd, regs) __asm__ __volatile__(#op " %" #regs ", %" #regd)
