...
void rectfill_add_32(BITMAP *where, int x1, int y1, int x2, int y2, Pixel colour, int fact) {
	...
	RECT_Y_LOOP(RECT_X_LOOP(*p = addColorsCrude_32(*p, col)))
}
...
void hline_add_32(BITMAP *where, int x1, int y1, int x2, Pixel colour, int fact) {
	...
	RECT_X_LOOP(*p = addColorsCrude_32(*p, col))
}
...
void drawSpriteLine_add_32(BITMAP *where, BITMAP *from, int x, int y, int x1, int y1, int x2, int fact) {
	...
	if (fact >= 255) {
		...
		SPRITE_X_LOOP(Pixel s = *src; if (s & 0xFF000000) *dest = addColorsCrude_32(*dest, s);)
	} else if (fact > 0) {
		SPRITE_X_LOOP(Pixel s = *src; if (s & 0xFF000000) *dest = addColorsCrude_32(*dest, scaleColor_32(s, fact));)
	}
}
...
void drawSpriteLine_add_8(BITMAP *where, BITMAP *from, int x, int y, int x1, int y1, int x2, int fact) {
	...
	if (fact >= 255) {
		SPRITE_X_LOOP_ALIGN(4, 4, *dest = addColorsCrude_8_4(*dest, *src), *dest = addColorsCrude_8_4(*dest, *src))
	} else if (fact > 0) {
		SPRITE_X_LOOP_ALIGN(4, 4, *dest = addColorsCrude_8_4(*dest, scaleColor_8_4(*src, fact)),
