...
void drawSprite_multsec_32_with_8(BITMAP *where, BITMAP *from, BITMAP *secondary, int x, int y, int sx, int sy,
	...
	if (bitmap_color_depth(from) != 32 || bitmap_color_depth(secondary) != 8)
		return;

	CLIP_SPRITE_REGION_SEC();

	SPRITE_Y_LOOP_SEC(SPRITE_X_LOOP_SEC(*dest = scaleColor_32(*src, *sec);))
}
...
void drawSprite_mult_8_to_32(BITMAP *where, BITMAP *from, int x, int y, int cutl, int cutt, int cutr, int cutb) {
	...
	SPRITE_Y_LOOP(SPRITE_X_LOOP_T(*dest = scaleColor_32(*dest, *src);))
}
