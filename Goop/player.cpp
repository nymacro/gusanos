...
Player::Player(boost::shared_ptr<PlayerOptions> options, BaseWorm *worm)
	: BasePlayer(options, worm), aimingUp(false), aimingDown(false), changing(false), jumping(false),
	  walkingLeft(false), walkingRight(false), m_actionActive(), m_actionIntensity()
#ifndef DEDSERV
	  ,
	  m_viewport(0)
#endif
		...
		case JUMP: {
			...
			if (m_worm) {
				if (m_worm->isActive()) {
					...
					std::cout << "[DEBUG] Player::actionStart JUMP with inactive worm, sending RESPAWN. m_node="
							  << (void *)m_node << " isActive=" << m_worm->isActive() << std::endl;
