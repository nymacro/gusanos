...
void Client::ZCom_cbConnectResult(ZCom_ConnID _id, eZCom_ConnectResult _result, ZCom_BitStream &_reply) {
	if (_result != eZCom_ConnAccepted) {
		...
		for (auto const &m : game.modList)
			DLOG("    modList entry: '" << m << "'");
...
void Client::ZCom_cbDataReceived(ZCom_ConnID id, ZCom_BitStream &data) {
	...
	switch (event) {
		case Network::ClientEvents::LuaEvents: {
			for (int t = Network::LuaEventGroup::Game; t < Network::LuaEventGroup::Max; ++t) {
				int c = data.getInt(8);
				for (int i = 0; i < c; ++i) {
...
void Client::ZCom_cbNodeRequest_Dynamic(ZCom_ConnID _id, ZCom_ClassID _requested_class, ZCom_BitStream *_announcedata,
										int _role, ZCom_NodeID _net_id) {
	...
	if (_requested_class == NetWorm::classID) {
		...
		if (!_announcedata) {
			console.addLogMsg("* ERROR: particle announce data missing");
			return;
		}
