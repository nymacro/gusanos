...
struct AddCuller : Culler<AddCuller> {
	AddCuller(Level &level, BITMAP *dest, BITMAP *source, int alpha, int dOffx, int dOffy, int sOffx, int sOffy,
			  Rect const &rect)
		: Culler<AddCuller>(rect), m_level(level), m_dest(dest), m_source(source), m_destOffx(dOffx), m_destOffy(dOffy),
		  m_sourceOffx(sOffx), m_sourceOffy(sOffy), m_alpha(alpha) {}
	...
	void line(int y, int x1, int x2) {
		drawSpriteLine_add(m_dest, m_source, x1 - m_destOffx, y - m_destOffy, x1 - m_sourceOffx, y - m_sourceOffy,
						   x2 - m_sourceOffx + 1, m_alpha);
	}
...
// TODO: optimize this
void Level::specialDrawSprite(Sprite *sprite, BITMAP *where, const IVec &pos, const IVec &matPos,
							  BlitterContext const &blitter) {
...
void Level::culledDrawSprite(Sprite *sprite, Viewport *viewport, const IVec &pos, int alpha) {
	...
	{
		AddCuller addCuller(*this, viewport->dest, renderBitmap, alpha, off.x, off.y, loff.x, loff.y, r);
...
void Level::culledDrawLight(Sprite *sprite, Viewport *viewport, const IVec &pos, int alpha) {
	...
	{
		AddCuller addCuller(*this, viewport->fadeBuffer, renderBitmap, alpha, off.x, off.y, loff.x, loff.y, r);
	...
	if (effect && effect->mask) {
		...
		for (int y = 0; y < tmpMask->m_bitmap->h; ++y)
			for (int x = 0; x < tmpMask->m_bitmap->w; ++x) {
				...
				bool isBlack =
					(maskDepth == 32) ? (((colour & 0xFF000000) != 0) && ((colour & 0x00FFFFFF) == 0)) : (colour == 0);
