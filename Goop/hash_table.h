...
struct hash<std::string> {
	Hash operator()(std::string const &key) const {
		...
		Hash h = 0;
		for (; left > sizeof(Hash); left -= sizeof(Hash), d += sizeof(Hash)) {
			h ^= *(Hash *)d;
		}
...
class HashTable {
  public:
	struct Node : public std::pair<const KeyT, ValueT> {
		Node(KeyT const &key, ValueT const &value) : std::pair<const KeyT, ValueT>(key, value), next(0) {}

		Node(KeyT const &key) : std::pair<const KeyT, ValueT>(key, ValueT()), next(0) {}
	...
	struct iterator {
	  ...
	  private:
		iterator(Node *slot_) : slot(slot_) {}

		Node *slot;
	};

	HashTable(size_t initialSizeOrder = 3, HashFunc hashFunc_ = HashFunc())
		: count(0), sizeOrder(initialSizeOrder), hashFunc(hashFunc_) {
		if (sizeOrder > 17)
			sizeOrder = 17;
