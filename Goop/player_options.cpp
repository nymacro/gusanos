...
PlayerOptions::PlayerOptions(std::string const &name_)
	: name(name_), uniqueID(0), team(-1), m_nameChanged(false), m_colorChanged(false) {
...
void PlayerOptions::registerInConsole(int index) {
	...
	console.registerVariables()("P" + sindex + "_AIM_ACCEL", &aimAcceleration, AngleDiff(0.17))(
		"P" + sindex + "_AIM_FRICTION", &aimFriction, pow(0.89, 0.7))("P" + sindex + "_AIM_SPEED", &aimMaxSpeed,
																	  AngleDiff(1.7))("P" + sindex + "_VIEWPORT_FOLLOW",
																					  &viewportFollowFactor, 0.1)(
		"P" + sindex + "_ROPE_ADJUST_SPEED", &ropeAdjustSpeed, 0.5)("P" + sindex + "_NAME", &name, "GusPlayer",
