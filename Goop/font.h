...
class Font {
  public:
	...
	struct CharInfo {
		...
		CharInfo(Rect const &rect_, int spacing_) : rect(rect_), spacing(spacing_), subBitmap(0) {
			width = rect.getWidth();
	...
	struct CharFormatting {
		struct Item {
			...
			Item(Color const &color_) : color(color_) {}

			Color color;
		};

		CharFormatting() : loc(0) {}

		CharFormatting(Item item_) : cur(item_), loc(0) {}
	...
	void draw(BITMAP *where, std::string const &text, int x, int y, int spacing = 0, int fact = 256, int cr = 255,
			  int cg = 255, int cb = 255, int flags = 0) {
		CharFormatting format(CharFormatting::Item(Color(cr, cg, cb)));
		draw(where, text.begin(), text.end(), x, y, format, spacing, fact, flags);
	}

	void draw(BITMAP *where, std::string const &text, int x, int y, CharFormatting &format, int spacing = 0,
			  int fact = 256, int flags = 0) {
		draw(where, text.begin(), text.end(), x, y, format, spacing, fact, flags);
	}

	void draw(BITMAP *where, std::string::const_iterator b, std::string::const_iterator e, int x, int y,
			  int spacing = 0, int fact = 256, int cr = 255, int cg = 255, int cb = 255, int flags = 0) {
		CharFormatting format(CharFormatting::Item(Color(cr, cg, cb)));
