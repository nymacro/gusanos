...
void GContext::init() {
	...
	console.registerCommands()("GUI_LOADXML", cmdLoadXML)("GUI_LOADGSS", cmdLoadGSS)("GUI_GSS", cmdGSS)("GUI_FOCUS",
																										cmdFocus);
}
...
bool GContext::eventKeyDown(int k) {
	...
	if (focus && focus->isVisibile() && focus->doKeyDown(k)) {
		...
		if (parent && parent->isVisibile()) {
			switch (k) {
				case KEY_UP: {
					if (Wnd *next = parent->findClosestChild(focus, Wnd::Up)) {
						setFocus(next);
					}
				} break;

				case KEY_DOWN: {
					if (Wnd *next = parent->findClosestChild(focus, Wnd::Down)) {
						setFocus(next);
					}
				} break;

				case KEY_LEFT: {
					if (Wnd *next = parent->findClosestChild(focus, Wnd::Left)) {
						setFocus(next);
					}
				} break;

				case KEY_RIGHT: {
					if (Wnd *next = parent->findClosestChild(focus, Wnd::Right)) {
						setFocus(next);
					}
				} break;
			}
...
// Draws a box
void SDLRenderer::drawBox(Rect const &rect, RGB const &color, RGB const &borderLeftColor, RGB const &borderTopColor,
						  RGB const &borderRightColor, RGB const &borderBottomColor) {
...
void SDLRenderer::drawFrame(Rect const &rect, RGB const &color) {
...
// Draws a box
void SDLRenderer::drawBox(Rect const &rect, RGB const &color) {
	blitter.rectfill(gfx.buffer, rect.x1, rect.y1, rect.x2, rect.y2, allegroColor(color));
...
// Draws text
void SDLRenderer::drawText(BaseFont const &font, std::string const &str, ulong flags, ulong x, ulong y,
						   RGB const &aColor) {
	const int spacing = 0;
...
std::pair<int, int> SDLRenderer::getTextDimensions(BaseFont const &font, std::string::const_iterator b,
...
int SDLRenderer::getTextCoordToIndex(BaseFont const &font, std::string::const_iterator b, std::string::const_iterator e,
									 int x) {
	if (GusanosFont const *f = dynamic_cast<GusanosFont const *>(&font)) {
...
void SDLRenderer::drawSprite(BaseSpriteSet const &spriteSet, int frame, ulong x, ulong y, ulong left, ulong top,
							 ulong bottom, ulong right) {
	if (GusanosSpriteSet const *s = dynamic_cast<GusanosSpriteSet const *>(&spriteSet)) {
...
void SDLRenderer::drawSkinnedBox(BaseSpriteSet const &skin, Rect const &rect, RGB const &backgroundColor) {
	if (GusanosSpriteSet const *s = dynamic_cast<GusanosSpriteSet const *>(&skin)) {
		...
		drawBox(Rect(rect.x1 + skinWidth, rect.y1 + skinHeight, rect.x2 - skinWidth - 1, rect.y2 - skinHeight - 1),
		backgroundColor); for(y = rect.y1 + skinHeight; y < rect.y2 - skinHeight * 2; y += skinHeight)
		{
