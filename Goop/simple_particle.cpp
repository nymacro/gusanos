...
void SimpleParticle::think() {
	spd.y += gravity;
	Vec nextPos = pos + spd;
	if (!game.level.getMaterial(int(nextPos.x), int(nextPos.y)).particle_pass || --timeout == 0)
		deleteMe = true;
	else
		pos = nextPos;
...
void SimpleParticle32::draw(Viewport *viewport) {
	...
	if ((unsigned int)rPos.x < (unsigned int)where->w && (unsigned int)rPos.y < (unsigned int)where->h)
