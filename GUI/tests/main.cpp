...
BOOST_AUTO_TEST_CASE(child_attribute_roundtrip)
{
	...
	BOOST_CHECK_NO_THROW(execLua(
		"local c = gui_root():child('attr_child'); "
...
{
	execLua(
		"local c = gui_window({id='same_obj'}); "
		"gui_root():add(c)");
	Wnd* root = g_context.getRoot();
	Wnd* found = root->getChildByName("same_obj");
	BOOST_REQUIRE(found != nullptr);
	BOOST_CHECK_NO_THROW(execLua(
		"local c = gui_root():child('same_obj'); "
		"assert(c:is_visible())"));
}
...
BOOST_AUTO_TEST_CASE(parent_child_visibility_propagation)
{
	...
	execLua(
		"local c = gui_window({id='vis_child'}); "
		"gui_root():add(c); "
...
{
	BOOST_CHECK_NO_THROW(execLua(
		"local l = gui_label({id='txt_label'}); "
		"gui_root():add(l); "
...
{
	BOOST_CHECK_NO_THROW(execLua(
		"local l = gui_label({id='empty_label'}); "
		"gui_root():add(l); "
		"l:set_text(''); "
		"assert(l:text() == '')"));
}
...
{
	BOOST_CHECK_NO_THROW(execLua(
		"local w = gui_window({id='active_w'}); "
		"gui_root():add(w); "
		"w:activate(); "
