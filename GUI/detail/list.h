...
struct ListNode : public LNodeImp<ListNode> {
	...
	ListNode(std::string const &text)
		: selected(false), expanded(true), parent(0), visibleChildren(0), level(0), list(0), m_destroyed(false) {
...
class List : public Wnd {
  public:
	...
	struct ColumnHeader {
		ColumnHeader(std::string const &name_, double widthFactor_) : name(name_), widthFactor(widthFactor_) {}
	...
	List(Wnd *parent, /*std::string const& tagLabel, std::string const& className,
	  std::string const& id,*/
		...
		: Wnd(parent, attributes, "list"), m_RootNode("root"), m_Base(0), m_basePos(0), m_MainSel(0),
		  m_totalWidthFactor(0.0), m_visibleChildren(0) {
	...
	void clear() {
		...
		for (std::map<ListNode *, LuaReference>::iterator i = m_RootNode.m_ownedNodes.begin(),
														  e = m_RootNode.m_ownedNodes.end();
			 i != e; ++i)
	...
	struct NumericLT {
		NumericLT(unsigned int column_) : column(column_) {}

		bool operator()(ListNode *a, ListNode *b) {
			return lexical_cast<int>(a->getText(column)) > lexical_cast<int>(b->getText(column));
		}
	...
	struct LuaLT {
		LuaLT(LuaContext &context_, LuaReference comparer) : context(context_), comp(comparer) {}
  ...
  private:
	...
	struct ListFormatting {
		ListFormatting()
			: headerColor(RGB(170, 170, 255)), selectionColor(RGB(170, 170, 255)), selectionFrameColor(RGB(0, 0, 0)),
			  indent(3.0) {}
