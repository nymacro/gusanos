...
void ListNode::render(Renderer *renderer, long &y) {
	...
	if (selected || list->m_MainSel == node_iter_t(this)) {
		...
		if (selected) {
			renderer->drawBox(r, list->m_listFormatting.selectionColor);
		}

		if (list->m_MainSel == node_iter_t(this)) {
			renderer->drawFrame(r, list->m_listFormatting.selectionFrameColor);
		}
	}
	...
	for (; i != columns.end(); ++i, ++h) {
		renderer->drawText(*list->m_font, *i, BaseFont::CenterV, long(x), y + halfRowHeight,
...
bool List::render() {
	...
	{
		renderer->drawBox(Rect(getRect().x1 + 1, getRect().y1 + 1, getRect().x2 - 1, getRect().y1 + rowHeight - 1),
		...
		for (std::vector<ColumnHeader>::const_iterator i = m_columnHeaders.begin(); i != m_columnHeaders.end(); ++i) {
...
bool List::mouseDown(ulong x, ulong y, Context::MouseKey::type button) {
	if (button == Context::MouseKey::Left) {
		...
		if (m_Base) {
			...
			if (range && m_MainSel && m_MainSel != newSel && m_MainSel->parent == newSel->parent) {
				bool toggle = false;
