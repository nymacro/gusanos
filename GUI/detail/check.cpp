...
bool Check::render() {
	...
	if (m_formatting.background.skin) {
		...
		renderer->drawBox(r, m_formatting.background.color, m_formatting.borders[0].color,
	...
	if (m_font)
		renderer->drawText(*m_font, m_text, BaseFont::CenterV, getRect().x1 + h + 4, getRect().centerY(),
